    ...
    )
{
    ...
    // parse buffer
    uint16_t buf_hdr = (buffer[BCL_HEADER_INDEX] << 8) | buffer[BCL_HEADER_INDEX  + 1];
    ...
    uint8_t computed_crc = compute_crc8(buffer + PAYLOAD_INDEX, length - PACKET_MIN_SIZE);
    ...
    )
{
    ...
    if (header.PayloadSize != pkt->Header.PayloadSize)
